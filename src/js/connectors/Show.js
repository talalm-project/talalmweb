...
import { faArrowLeft, faCircleInfo, faCloudArrowUp, faDiagramProject, faFlask, faPaperPlane, faPenToSquare, faServer, faTrash } from "@fortawesome/free-solid-svg-icons";
...
const ConnectorsShow = () => {
  ...
  const handleDelete = () => {
    ...
    ConnectorService.deleteConnector(connector.id)
      .then(() => {
        navigate("/connectors");
      })
      .catch((error) => {
        ...
        setErrorMessage(error.response?.data?.message || "Unable to delete connector.");
  ...
  return (
    <div className="talalm-connector-show d-flex flex-column gap-4">
      ...
      <ConfirmationModal
        ...
        content={`Delete ${connector?.name || "this connector"}? This action cannot be undone.`}
        loadingContent="Deleting connector..."
